...
const statusStyles = {
  [LoanStatus.ACTIVE]: "bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20",
  [LoanStatus.RETURNED]: "bg-gray-500/10 text-gray-600 hover:bg-gray-500/20",
  [LoanStatus.OVERDUE]: "bg-destructive/10 text-destructive hover:bg-destructive/20",
  ...
  return (
    <Card className={cn("flex flex-col", className)}>
      ...
      <CardContent className="flex-1 space-y-1.5 overflow-auto p-3">
        {loans.map((loan, i) => {
            const daysRemaining = new Date(loan.return_date).getTime() - new Date(Date.now()).getTime();
            return (
          <Card key={i} className="transition-colors hover:bg-accent/50">
            <CardContent className="flex items-center justify-between p-2.5">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">{loan.book.title}</p>
                  <p className="text-xs text-muted-foreground">{loan.book.publisher.name}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Badge variant="secondary" className={cn(statusStyles[loan.status], "border-0 text-xs")}>
