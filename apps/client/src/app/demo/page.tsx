...
import { DemoRoleCard } from '@repo/ui';
			...
			if (result?.data) {
				if (
					...
					setTimeout(() => {
						...
						window.location.href = `${baseUrl}${tenant.subdomain}.${process.env.NEXT_PUBLIC_BASE_URL}/dashboard`;
					}, 1000);
				} else if (
					...
					if (demoTenant) {
						...
						setTimeout(() => {
							const baseUrl =
								process.env.NODE_ENV === 'production' ? 'https://' : 'http://';
							window.location.href = `${baseUrl}demo.${process.env.NEXT_PUBLIC_BASE_URL}/dashboard`;
						}, 1000);
					}
				}
			}
