import { IGeneric, BaseQueriesDto } from '@repo/common';
...
export class QueryHelper {
  ...
  static applyBaseFilters<TEntity extends IGeneric>(
    ...
    // Filtros de fecha
    if (queries.fromDate) {
      qb.andWhere(`${alias}.createdAt >= :fromDate`, { fromDate: queries.fromDate });
    }
    if (queries.toDate) {
   ...
   */
  static async getPaginatedResponse<TEntity extends IGeneric, TResponse = TEntity>(
    qb: SelectQueryBuilder<TEntity>,
    queries: BaseQueriesDto<TEntity>,
    ...
    return {
      items: transformFn ? rows.map(transformFn) : (rows as unknown as TResponse[]),
      meta: {
        current_page: page,
        per_page: limit,
